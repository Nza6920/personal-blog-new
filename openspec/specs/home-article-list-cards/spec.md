...
### Requirement: Home page article cards use article cover images
The system SHALL use the article cover image as the left cover source when it exists, and fall back to `uploads/images/system/default.jpg` when it does not.

#### Scenario: Prefer article cover image for the list cover
- **WHEN** the home page article list is rendered
- **THEN** each article card uses the topic `cover_img` value as the cover source when available
- **AND** the card falls back to `uploads/images/system/default.jpg` when no article cover image exists
